...
and progress. Be warm and genuine.`,
        ...
        try {
          if (typeof window !== "undefined" && window.dispatchEvent) {
            window.dispatchEvent(
              new CustomEvent("gemini-download-progress", { detail: pct }),
            );
          }
        } catch (err) {
  ...
  hasUserActivation() {
    ...
    console.log(
      "[GeminiAPI] Step 2: User activation confirmed (popup active):",
      activated,
    );
  ...
  async checkAvailability() {
    try {
      if (!("Summarizer" in self)) {
        console.warn(
          "Old chrome version or antiquated hardware detected. This is problem indicating hardware or chrome version, not extension issue",
        );
      }

      // CRITICAL: Same options as in create() method
      // Pass CREATE_OPTIONS to availability() to ensure the check matches create()
  ...
  async initSession() {
    ...
    try {
      console.log(
        "[GeminiAPI.initSession] *** CREATING SESSION (THIS TRIGGERS DOWNLOAD IF NEEDED) ***",
      );

      // Timeout warning if creation takes >30 seconds
      const timeoutWarning = setTimeout(() => {
        console.warn(
          "[GeminiAPI.initSession] ⚠️ TIMEOUT WARNING: Session creation taking >30 seconds",
        );
        console.warn(
          "[GeminiAPI.initSession] ⚠️ Download might still be in progress, check chrome://on-device-internals",
        );
      ...
      console.log(
        `[GeminiAPI.initSession] ✓✓✓ SESSION CREATED (took ${elapsedSeconds}s) ✓✓✓`,
      );
      return this.session;
    } catch (error) {
      const elapsedSeconds = Math.round((Date.now() - startTime) / 1000);
      console.error(
        `[GeminiAPI.initSession] *** ERROR CREATING SESSION (after ${elapsedSeconds}s) ***`,
      );
      ...
      if (error.message?.includes("not available")) {
        console.error(
          "[GeminiAPI.initSession] ⚠ Model not available on this device",
        );
      }
  ...
  async pollForAvailability(maxWaitMs = 300000) {
    ...
    console.log(
      "[GeminiAPI] Step 5: Polling for model availability (max wait: " +
        Math.round(maxWaitMs / 1000) +
        "s)",
    );
    ...
    while (Date.now() - startTime < maxWaitMs) {
      try {
        ...
        // normalize a few expected availability strings from the platform
        const avail = (availability || "").toString();
        if (avail === "readily" || avail === "ready" || avail === "available") {
          console.log(
            `[GeminiAPI] ✓ Model available for language server (after ${elapsedSeconds}s)`,
          );
          try {
            if (typeof window !== "undefined" && window.dispatchEvent) {
              window.dispatchEvent(new CustomEvent("gemini-model-ready"));
            ...
            console.warn(
              "[GeminiAPI] Could not dispatch ready event from poll",
              err,
            );
          }
          return {
        ...
        // Treat 'downloadable' and 'after-download' (and similar) as "still downloading"
        if (
          ...
          console.log(
            `[GeminiAPI] Model still downloading... (${elapsedSeconds}s elapsed, checking again in 5s)`,
          );
        ...
        if (availability === "no") {
          console.warn(
            "[GeminiAPI] Model not available on this device (likely not supported)",
          );
          return {
            available: false,
            status: "unavailable",
    ...
    return {
      ...
      status: "timeout",
      message:
        "Model download timeout. Check chrome://on-device-internals for details",
    };
  },
  ...
  async generateAffirmation(tasks) {
    console.log(
      "[GeminiAPI.generateAffirmation] *** ENTRY - tasks:",
      tasks.length,
    );

    try {
      console.log(
        "[GeminiAPI.generateAffirmation] === Starting affirmation generation ===",
      );
      ...
      const availability = await this.checkAvailability();
      console.log(
        "[GeminiAPI.generateAffirmation] Availability:",
        availability,
      );

      if (availability === "unavailable") {
        console.warn(
          "[GeminiAPI.generateAffirmation] ✗ Model not available on this device",
        );
        return {
      ...
      console.log(
        `[GeminiAPI.generateAffirmation] Creating session... (availability: ${availability})`,
      );
      ...
      console.log(
        "[GeminiAPI.generateAffirmation] Sending prompt to summarizer...",
      );
      console.log("[GeminiAPI.generateAffirmation] Context:", context);
      const result = await session.summarize(prompt, {
        context: context,
        outputLanguage: this.CREATE_OPTIONS.outputLanguage,
      });

      console.log(
        "[GeminiAPI.generateAffirmation] ✓✓✓ Affirmation generated successfully ✓✓✓",
      );
