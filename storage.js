...
const StorageManager = {
  ...
  async isNewDay() {
    try {
      ...
      console.log('[Storage.isNewDay] Last stored date:', lastDate, 'Today:', this.getTodayString());
  ...
  async getTodaysTasks() {
    try {
      ...
      console.log('[Storage.getTodaysTasks] Found', data[this.STORAGE_KEY].items.length, 'tasks');
  ...
  async addTask(text) {
    ...
    console.log('[Storage.addTask] Saving to storage... Tasks count now:', tasks.items.length);
  ...
  async toggleTask(taskId) {
    try {
      ...
      await chrome.storage.local.set({ [this.STORAGE_KEY]: tasks });
      console.log('[Storage.toggleTask] Task toggled:', taskId, 'completed:', task.completed);
  ...
  async getCompletedTasks() {
    try {
      ...
      console.log('[Storage.getCompletedTasks] Found', completed.length, 'completed tasks');
      return completed;
    } catch (error) {
