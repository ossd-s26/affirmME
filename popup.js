...
class ChecklistApp {
  ...
  async init() {
    ...
    console.log(
      "[App.init] Event listeners will be mounted when model is ready",
    );
    ...
    try {
      ...
      // If model is already ready, show the app immediately
      if (this.apiStatus === "readily" || this.apiStatus === "available") {
        this.showApp();
      } else {
        // Show loading screen and wait for model-ready events
        this.showLoading();
      }
    } catch (error) {
  ...
  showLoading() {
    ...
    // progress listener
    this._geminiProgressHandler = (ev) => {
      const pct = Math.round(ev.detail || 0);
      const pctEl = document.getElementById("downloadPct");
      if (pctEl) pctEl.textContent = pct + "%";
      // also update banner message
      this.showStatusBanner("info", `Downloading AI model... ${pct}%`, true);
    };
    window.addEventListener(
      "gemini-download-progress",
      this._geminiProgressHandler,
    );

    // ready listener
    this._geminiReadyHandler = () => {
    ...
    window.addEventListener("gemini-model-ready", this._geminiReadyHandler);
  }
  ...
  showApp() {
    ...
    // mount app listeners and render tasks
    try {
      this.setupEventListeners();
      // load tasks then update progress/streak UI
      this.loadAndRenderTasks().then(async () => {
        try {
          const prog = await StorageManager.getProgress();
          this.updateProgressUI(prog.completed, prog.total);
          const streak = await StorageManager.getStreak();
          this.updateStreakUI(streak);
        } catch (err) {
          console.warn("[showApp] Could not update progress/streak", err);
        }
      });
      this.setupDateDisplay();
      this.hideStatusBanner();
    } catch (err) {
      console.error("[App.showApp] Error mounting app:", err);
    }
  }
  ...
  updateProgressUI(completed, total) {
    ...
    const pctEl = document.getElementById("downloadPct");
    // Reuse downloadPct element if present to show progress bar-like percent for tasks
    if (pctEl) pctEl.textContent = `${pct}%`;
  }
  ...
  async handleToggleTask(taskId) {
    ...
    try {
      console.log(
        "[HandleToggleTask] STEP 1: Calling StorageManager.toggleTask()...",
      );
      const { task, isFirstCompletion } =
        await StorageManager.toggleTask(taskId);
      console.log(
        "[HandleToggleTask] STEP 2: Got response, isFirstCompletion=",
        isFirstCompletion,
        "completed=",
        task.completed,
      );
      ...
      try {
        ...
        console.warn(
          "[HandleToggleTask] Could not update progress/streak UI",
          err,
        );
      }

      // Decoupled: do not auto-generate affirmations on task completion
      if (!task.completed) {
        console.log(
          "[HandleToggleTask] STEP 4B: Task unchecked, hiding affirmation",
        );
        // Task unchecked - hide affirmation
        document.getElementById("affirmationSection").classList.remove("show");
      } else {
        console.log(
          "[HandleToggleTask] STEP 4C: Not first completion or not completed",
        );
      }
  ...
  async generateAndDisplayAffirmation() {
    ...
    try {
      ...
      console.log(
        "[GenerateAffirmation] STEP 3: Got completed tasks:",
        completedTasks.length,
      );

      console.log(
        "[GenerateAffirmation] STEP 4: Calling GeminiAPI.generateAffirmation()...",
      );
      ...
      console.log(
        "[GenerateAffirmation] Showing fallback affirmation due to error",
      );
    } finally {
      console.log(
        "[GenerateAffirmation] STEP 7: Finally block, setting affirmationInProgress = false",
      );
      this.affirmationInProgress = false;
    }
  }
  ...
  async loadAndRenderTasks() {
    ...
    try {
      ...
      console.warn(
        "[loadAndRenderTasks] Could not update progress/streak UI",
        err,
      );
    }
  }
  ...
  renderTasks(tasks) {
    ...
    tasks.forEach((task, index) => {
      ...
      console.log(
        "[RenderTasks] Attaching click listener to delete button:",
        task.id,
      );
